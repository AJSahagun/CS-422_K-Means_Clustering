...
def plot_clustering_analysis_3d(X_scaled, df, cluster_labels, true_labels, cluster_centers, wcss_values, conf_matrix, scaler=None):
    """
    3D visualization of clustering analysis
    ...
    scaler : sklearn.preprocessing.StandardScaler, optional
        Scaler used for feature scaling; if provided, centroids are inverse-transformed
    """
    ...
    # 2. 3D Clustering Visualization
    ax2 = fig.add_subplot(2, 2, 2, projection='3d')
    # If scaler is provided, transform cluster centers back to original scale
    if scaler is not None:
        cluster_centers_orig = scaler.inverse_transform(cluster_centers)
    else:
        cluster_centers_orig = cluster_centers

    # Plot data points with colors representing cluster labels
    ax2.scatter(df['SepalLengthCm'], df['SepalWidthCm'], df['PetalLengthCm'],
                c=cluster_labels, cmap='viridis', s=100, alpha=0.6)
    # Plot centroids
    ax2.scatter(cluster_centers_orig[:, 0], cluster_centers_orig[:, 1], cluster_centers_orig[:, 2],
                s=300, c='red', marker='X', linewidth=2, label='Centroids')
    ax2.set_title('K-means Clustering (3D)', fontsize=14, pad=15)
    ...
    # 3. 3D True Labels Visualization
    ...
    ax3.set_title('Actual Species Distribution (3D)', fontsize=14, pad=15)
    ...
    # 4. Confusion Matrix (2D heatmap)
    ax4 = fig.add_subplot(2, 2, 4)
    sns.heatmap(conf_matrix, annot=True, fmt='d', cmap='Blues',
                xticklabels=unique_species,
                yticklabels=unique_species, ax=ax4)
...
def calculate_wcss(X_scaled, kmeans_class, max_k=10):
    ...
    X_scaled : array-like
        Scaled feature data
    kmeans_class : class
        KMeans class to use (either custom or from sklearn)
    max_k : int
